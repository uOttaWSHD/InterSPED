...
interface AnalysisPageProps {
  companyName: string;
  jobUrl: string;
  onComplete: (initialResponse?: string, sessionId?: string) => void;
  onFail: () => void;
}
  ...
  useEffect(() => {
    ...
    const run = async () => {
      try {
        ...
        if (interviewRes.status !== 200 || !interviewData.session_id) {
          setError(`Failed to start interview (Status ${interviewRes.status}). Please try again.`);
          setTimeout(() => {
            if (isMounted) onFail();
          }, 4000);
          return;
